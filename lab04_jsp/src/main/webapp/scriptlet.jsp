...
<body>
    ...
    <main>
        ...
        <%--
        scriptlet : JSP안에서 자바 코드들(_jspService 메서드안에서 실행되는 코드들)을 작성하기 위한 태그.
        지역변수선언&초기화, 객체생성, 메서드 호출, 조건문, 반복문, ...
         --%>
         ...
         <h2>scriptlet &amp; expression을 사용한 nuordered list</h2>
         <ul>
        <%--<li>이름</li>아이템을 반복문으로 만들기 --%>
        <% for(Contact c : data) { %>
            <li> <%= c.toString() %></li>       
        <% } %>
         </ul>
         ...
         <dl>
         <%-- <dt>이름</dt> <dd>전화번호</dd> <dd>이메일</dd> 반복문 만들기 --%>
         <% for(Contact c : data) { %>
            <dt><%= c.getName() %></dt>
            <dd><%= c.getPhone() %></dd>
            <dd><%= c.getEmail() %></dd>
